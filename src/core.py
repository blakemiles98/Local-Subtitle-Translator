...
def process_one_video(
    ...
    if not src_nllb:
        ...
        if status:
            status("Translation skipped", f"Detected '{detected_lang}' but no mapping. Wrote {fallback_source_srt_path.name}")
    ...
    english_srt = translator.translate_srt(source_srt, batch_size=20)
    if status:
        status("Finalize", f"Writing English SRT: {video_path.name}")
